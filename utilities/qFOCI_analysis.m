function [error] = qFOCI_analysis(analysisFolder, resultsFolder, saveImg, params, onylOne, filename)
%
%
%
%
...
%     i_dec = decimgs(i_file);
    
%     fprintf('\nAnalysing files:\nGFP\t:\t%s\nDec\t:\t%s\nTrans\t:\t%s\n', files(i_gfp).name, files(i_dec).name, files(i_trans).name);
    if ~two_sets
